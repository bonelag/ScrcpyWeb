    ...
    implements KeyEventListener, InteractionHandlerListener
{
    ...
    public onDisplayInfo = async (infoArray: DisplayCombinedInfo[]): Promise<void> => {
        if (!this.player) {
            return;
        }
        ...
        if (!this.joinedStream) {
            const { videoSettings, fitToScreen } = await (this.player.constructor as typeof BasePlayer).loadSettingsAsync(
                this.params.udid,
                info.displayInfo,
            );
            this.player.setVideoSettings(videoSettings, fitToScreen, false);
