...
export interface PlayerClass {
    ...
    loadSettingsAsync(deviceName: string, displayInfo?: DisplayInfo): Promise<{ videoSettings: VideoSettings, fitToScreen: boolean }>;
    saveVideoSettings(
        deviceName: string,
...
export abstract class BasePlayer extends TypedEmitter<PlayerEvents> {
    ...
    public static getVideoSettingFromStorage(
        ...
        return this.parseVideoSettings(parsed, preferred);
    }

    protected static parseVideoSettings(parsed: any, preferred: VideoSettings): VideoSettings {
        const {
            displayId,
            crop,
            bitrate,
    ...
    public static async loadSettingsAsync(
        udid: string,
        displayInfo?: DisplayInfo,
    ): Promise<{ videoSettings: VideoSettings; fitToScreen: boolean }> {
        const preferred = this.getPreferredVideoSetting();
        let videoSettings = preferred;
        let fitToScreen = false;

        try {
            const response = await fetch(`/api/settings/${udid}`);
            if (response.ok) {
                const data = await response.json();
                if (data && Object.keys(data).length > 0) {
                    videoSettings = this.parseVideoSettings(data, preferred);
                    if (typeof data.fitToScreen === 'boolean') {
                        fitToScreen = data.fitToScreen;
                    }
                } else {
                    // Fallback to local storage if server has no data
                    videoSettings = this.getVideoSettingFromStorage(preferred, this.storageKeyPrefix, udid, displayInfo);
                    fitToScreen = this.getFitToScreenFromStorage(this.storageKeyPrefix, udid, displayInfo);
                }
            }
        } catch (e) {
        ...
        return { videoSettings, fitToScreen };
    }

    public static async saveSettingsAsync(
        udid: string,
        videoSettings: VideoSettings,
        fitToScreen: boolean,
        displayInfo?: DisplayInfo,
    ...
    public setScreenInfo(screenInfo: ScreenInfo): void {
        ...
        if (this.parentElement) {
            // Keep the parent stretched to its container; do not force it to the video pixel size
