...
export class HttpServer extends TypedEmitter<HttpServerEvents> implements Service {
    ...
    public async start(): Promise<void> {
        ...
        const settingsService = DeviceSettingsService.getInstance();
        this.mainApp.get('/api/settings/:udid', (req, res) => {
            const { udid } = req.params;
            const settings = settingsService.getSettings(udid);
            res.json(settings || {});
        });
        this.mainApp.post('/api/settings/:udid', (req, res) => {
            const { udid } = req.params;
            const settings = req.body;
            settingsService.saveSettings(udid, settings);
